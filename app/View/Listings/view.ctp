...
$options = array();
$options['cssFiles'] = array(/*'sweetThumbnails/style.css',*/'grassSky.css', "flexslider.css"); //i've re-included the template css file after the jquery file, so we can override any of the jquery styles that we don't like
$options['jsFiles'] = array(/*'sweetThumbnails.js',*/ "jquery.flexslider-min.js",'http://maps.google.com/maps/api/js?sensor=false', 'googleMaps.boundsbox.js');
...
<div class="imageDiv grid_8">
	...
	<div id="ps_loader" class="ps_loader"></div>
		<ul class="slides" style="display:none;">
			<?php foreach ($listing['ListingPhoto'] as $k=>$photo) {  ?>
				<li><img src="<?php echo $photo['url'];?>"/></li>
				<? } ?>
          </ul>
		<?php /* 
		<div id="ps_loader" class="ps_loader"></div>
